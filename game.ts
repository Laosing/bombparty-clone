...
const rooms = new Map<string, Map<keyof Room, any>>();

const admin: { name: string } = {
  name: "",
};

function connection(io: Server, socket: Socket) {
  ...
  function checkGameState() {
    ...
    const singlePlayer =
      _groups.length === 1 ? _groups[0][1].lives <= 0 : false;
  ...
  function getNextPlayer(collection: Map<string, Group>) {
    ...
    const groups = [...collection].filter(
      ([, val]) => val.members.size
    );
  ...
  function getRoom(isPrivate?: boolean): RoomProps & { room: Map<keyof Room, any> } {
    const room = rooms.get(_roomId) || rooms.set("null_room", new Map()).get("null_room")!;
